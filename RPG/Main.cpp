...
int main(){;
	...
	GameConditions gameConditions = GameConditions(map,*playChar);
	...
	while (!(playerController.get_last_input() != "q") != !gameConditions.lose() != gameConditions.win()){
		playerController.execute();
	}

	if (gameConditions.lose()){
		consoleOutputStream << "\nYou are DEAD!\nGAME OVER!" << '\n';
	}
	else if (gameConditions.win()){
